...
func getFdcRewards(db *gorm.DB, epochs data.RewardEpochs, submit2 []payload.Message, submitSignatures []payload.Message, finalizations []*data.Finalization) ([]ty.RewardClaim, error) {
	...
	for round := re.StartRound; round <= re.EndRound; round++ {
		...
		// Attestation req
		reqs := attestationRequestsByRound[round]
		...
		inflationOffer := epochs.Current.Offers.FdcInflation[0]
...
func getOffenders(
	bitVotes
map[ty.VoterSubmit]data.BitVote,
consensusSigs
...
func getConsensusBitVote(sigs
map[ty.VoterSigning]data.SigInfo, round
ty.RoundId, voters * data.VoterIndex) *string{
